...
class CurveUtils():
    ...
    def calculate_curve(body, bounding_box, triangle, axis, template, device):
        ...
        for value in height_values:
            ...
            measures = CurveUtils.calculate_distances(positions)
    ...
    def get_curves(selected_body, selected_measure, template, device):
        ...
        neck_rotation_matrix = euler_angles_to_matrix(tc.tensor([-17.5*(tc.pi/180),0,0]).to(device), "XYZ")
        neck_triangle = lambda x: tc.FloatTensor([[10, x, 10], [-10, x, 10], [0, x, -10]]).to(device) @ neck_rotation_matrix
